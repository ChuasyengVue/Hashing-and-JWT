...
class User {
  ...
  static async authenticate(username, password) { 
    ...
    if(!results){
      throw new ExpressError('Invalide Username/Password', 401)
    }
    let u = results.rows[0];

    return u && await bcrypt.compare(password, u.password);
  }
  ...
  static async get(username) { 
    ...
    if(!u){
      throw new ExpressError(`User is not found: ${username}`, 404)
    }
